...
exports.Listing.prototype.update_from_body = function (from_body) {
    for (key in valid_fields) {
        if (valid_fields.hasOwnProperty(key) && isString(from_body[key])) {
...
exports.Listing.prototype.update_from_object = function (from_object) {
    ...
    for (key in valid_fields) {
        if (valid_fields.hasOwnProperty(key) && isString(from_object[key])) {
...
exports.validate_dns = function (value, reqip, success, failure) {
    ...
    if (validator.isIP(value, "4")) {
        ...
        if (value === reqip) {
            ...
            console.error("dns.validate - failure (valid IPv4 but does not match request IP: "+value+" != "+reqip);
            return failure();
        }
    }
    ...
    if (checkdomain(value)) {
        dns.resolve6(value, function (err, addresses) {
            ...
            dns.resolve4(value, function (err, addresses) {
                ...
                console.error("dns.validate - failure (no matching addresses in DNS) for "+value);
                return failure();
            });
        });
    } else {
